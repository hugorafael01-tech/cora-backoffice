  ...
  return (
    <section className="px-5 pb-10 pt-6 md:px-8">
      <div className="mb-4 rounded border-l-[3px] border-brand-200 bg-warm-100 px-3.5 py-2.5 text-[12.5px] leading-relaxed text-warm-500">
        <strong className="font-semibold text-warm-700">Levain é refinável</strong> na calculadora
        acima a qualquer momento.{' '}
        <strong className="font-semibold text-warm-700">Produções de teste</strong> podem ser
        limpas antes do Alpha — não contam pro histórico.
      </div>
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="text-[12.5px] text-warm-500">
          {numProducoes === 0
            ? 'Defina o volume (qty maior que 0) pra criar produções.'
            : `${numProducoes} ${numProducoes === 1 ? 'produção planejada' : 'produções planejadas'} · etapas a gerar`}
        </div>
        <button
          onClick={onCriar}
          disabled={disabled}
        ...
        >
          {salvando ? 'Criando…' : 'Criar produções da semana'}
        </button>
      </div>
    </section>
  );
