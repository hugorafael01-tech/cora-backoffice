...
export function NovaReceitaTesteModal({ onAdd, onClose }: Props) {
  ...
  async function salvar() {
    ...
    try {
      ...
      if (modo === 'variacao') {
        if (!baseId) throw new Error('Selecione um pao base.');
        ...
        if (!nNome.trim()) throw new Error('Informe o nome do pao.');
    ...
    <ModalShell titulo="Nova receita de teste" eyebrow="mini-fluxo · status rascunho" onClose={onClose}>
      <div className="mb-4 grid grid-cols-2 overflow-hidden rounded-lg border-[1.5px] border-warm-300">
        <SegBtn ativo={modo === 'variacao'} onClick={() => setModo('variacao')}>
          Variacao de pao existente
          <span className="block text-[11px] font-normal text-warm-500">-&gt; nova versao</span>
        </SegBtn>
      ...
      {modo === 'variacao' ? (
        <div>
          <Campo label="Pao base">
            ...
            <Hint>A versao herda formato, grupo, ingredientes e etapas do pao base.</Hint>
          </Campo>
          <div className="grid grid-cols-2 gap-3.5">
            <CampoNum label="Hidratacao alvo" unidade="%" value={vHid} onChange={setVHid} />
            <CampoNum label="% de prefermento" unidade="%" value={vPref} onChange={setVPref} />
          </div>
          <CampoNum label="Peso da massa / pao" unidade="g" step={10} value={vPeso} onChange={setVPeso} />
          <Campo label="Notas">
            <textarea
              value={vNotas}
              onChange={(e) => setVNotas(e.target.value)}
              placeholder="o que esta testando nesta versao"
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-3.5">
            ...
            <CampoNum label="Peso da massa / pao" unidade="g" step={10} value={nPeso} onChange={setNPeso} />
          </div>
          <div className="grid grid-cols-2 gap-3.5">
            <CampoNum label="Hidratacao alvo" unidade="%" value={nHid} onChange={setNHid} />
            <CampoNum label="% de prefermento" unidade="%" value={nPref} onChange={setNPref} />
          </div>
          <Hint>
            Nasce sem ingredientes (massa = un x peso; levain definido depois no modulo Receitas).
            Hidratacao e prefermento ficam registrados nas notas.
          </Hint>
          <Campo label="Notas">
            <textarea
              value={nNotas}
              onChange={(e) => setNNotas(e.target.value)}
              placeholder="o que esta testando neste pao"
              className="min-h-[64px] w-full resize-y rounded-lg border-[1.5px] border-warm-300 px-3 py-2 text-[14px] text-warm-800 outline-none focus:border-brand-500"
            />
          </Campo>
        </div>
      ...
      <div className="mt-4 flex items-center justify-between gap-3 border-t border-warm-200 pt-4">
        ...
        >
          {salvando ? 'Adicionando…' : 'Adicionar a semana'}
        </button>
      </div>
    </ModalShell>
  );
