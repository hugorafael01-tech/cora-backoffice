...
**Última atualização:** 19 de junho de 2026 — **aba inicial da Produção derivada do estado do ciclo** (PR #54, `main` em `b718738`): `ProducaoAtualRedirect` injeta `?aba=` quando a URL não traz o param — `rascunho`/`A` → `volume`, `B` → `acompanhamento`, `C` → `registro`. Deep-link explícito tem prioridade. `derivaEstado` narrada para `Pick<Semana, 'status'|'data_corte'|'data_entrega'>` (backward-compatible). Tipo local `CicloComCorte = CicloLite & { data_corte }` encapsula o campo extra sem vazar pra `CicloLite` ou `ExpedicaoAtualRedirect`. Sem schema. Smokes validados (19/jun). Corrige BUGS item 3. Antes: titulo da etapa usa nome da receita (#52) + BUGS.md criado (#53).
