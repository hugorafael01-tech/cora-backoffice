...
**Última atualização:** 15 de junho de 2026 — **fix `criarVariacao`** (PR #47, `main` em `f4a3435`): `criarVariacao` agora sobrescreve a linha `agua-mineral` com `hidratacao_alvo / 100` após o fork, corrigindo ficha de bancada enganosa em variações de hidratação. Sem schema. `getLevainId` virou wrapper de `getIngredienteId(slug)`. Antes: Estado C fatia 1 + B2b-2 (#46).
