...
- **E1 (migration 0026): concluída e APLICADA (10/jun).** `CREATE TABLE entregas` — snapshot por ciclo, origem dupla `assinatura`/`avulso` (refs **lógicas sem FK** pra `weekly_orders`/`pedidos_pontuais` + CHECK de exatamente-um), `itens jsonb`, `regiao IN ('niteroi','rio')`, `status IN ('pendente','em_rota','entregue')` + `em_rota_at`/`entregue_at`, dois `UNIQUE` pra idempotência do gerador, RLS `admin_all` + trigger `set_updated_at`. Mergeada via **PR #43** e aplicada no SQL Editor (probes PRE/POS verificados, POS.1 = 21 colunas).
- **E2 (tela + gerador): concluída (10/jun) — branch `feat/e2-expedicao-ui`.** Sem schema. Rotas `/expedicao` → `/expedicao/atual` (resolve via `escolherCicloAtual`) → `/expedicao/:id`; nav Expedição **deixou de ser esmaecida** (sidebar + bottom-nav); `CicloSwitcher` ganhou prop `basePath` e é reusado com `/expedicao`.
  ...
  - **Smoke runtime PENDENTE:** gerador + transições passaram por tsc/lint/65 testes + build, mas não foram exercitados clicando com dados reais. Validar na tela antes de confiar.
