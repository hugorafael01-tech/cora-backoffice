...
**Última atualização:** 15 de junho de 2026 — **migration 0027 faturas** (`feat/schema-0027-faturas`, PR em aberto): `CREATE TABLE faturas` — materialização local do histórico de cobranças pagas. Enum `fatura_status_enum` (`pendente`/`paga`/`falha`/`cancelada`), FK → `subscriptions`, `periodo_referencia TEXT CHECK (YYYY-MM com mês validado)`, snapshot `qty_paes`/`valor_paes`/`valor_frete` + `valor_total` real do Asaas, `asaas_payment_id TEXT UNIQUE` (idempotência do webhook + reconciliação), 3 índices + partial index `paid_at WHERE status = 'paga'` + trigger `trg_faturas_updated_at`. RLS espelha 0020: SELECT pra admin via `is_admin()`; escrita somente `service_role`; anon nada. Aplicada no SQL Editor; probes PRE/POS verificados. Antes: fix `criarVariacao` (#47).
